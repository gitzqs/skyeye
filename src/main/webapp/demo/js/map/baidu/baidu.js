(function() {
	...
	var xxCache = [];
	// 复杂的自定义覆盖物
	function cutsomOv(point, text, shi) {
		this._point = point;
		this._text = text;
		this._shi = shi;
	}
	...
	cutsomOv.prototype.initialize = function(map) {
		...
		if (this._shi) {
			...
			var companyName = "某某公司";
			div.style.width = 12 * companyName.length + "px";
			_a.appendChild(document.createTextNode(companyName));
			div.appendChild(_a);
		} else {
			...
			div.onclick = function(e) {
				...
				if (xx) {
					for ( var o in xx) {
						xxCache.push(new cutsomOv(new BMap.Point(xx[o].g.split(",")[0], xx[o].g.split(",")[1]), xx[o].n + "," + Math.floor(Math.random() * 100), true));
					}
					removeSomeOverlays(centerPointsCache);
